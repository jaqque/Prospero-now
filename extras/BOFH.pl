...
my $no_bofh; # Can't think of any situation in which this won't work..

sub bofh::get { 
    my $line = shift;
    unless ($line =~ /^bofh[!? ]*$/i or $line =~ /^give excuse\?*$/i
    	or $line =~ /^sqod$/i) { 
	return '';
   }

    unless (@excuses) { # read data unless it's been read already.
	print "Reading...\n";
	while (<DATA>) {
	    chomp;
	    push @excuses, $_; 
    ...
    if ($no_excuse) { # ..but just in case :-)
	return "No excuses here . . ." if $main::addressed;
    }
    
    my $excuse = $excuses[rand(@excuses)];

    return "Your excuse is: $excuse";
}
