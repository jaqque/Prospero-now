...
sub Extras {
    ...
    # jaqque++
    if (getparam('rickroll')) {
	if (my $resp = rickroll::get($message)) {
	    return $resp;
	}
    }
    ...
    # jaqque++
    if (getparam('twitter')) {
	if (my $resp = &twitter::get($message)) {
	    return $resp;
	}
    }
...
# modified to fork and generally search by oznoid
    ...
    if ($message =~ /^b[o0]+[?!]+(\s|$)/i) {
       # reply appropriately
       return 'Aaack! Stop scaring me, '.$who.'!'
    }

    # reply in kind
    if ($message =~ /^w[o0][o0]+t.*/i) {
       # yee haa!
       return 'Yee haww!'
    }
    ...
    if ($message =~ /(good(\s+fuckin[\'g]?)?\s+(bo(t|y)|g([ui]|r+)rl))|((bot|scooby)(\s|\-)?(snack|b(ee|33)r)|bz|bravo\s+zulu)/i) {
	...
	if ($message =~ /(thank(s| you)|danke)/i) {
	    if ($msgType =~ /public/) {
		if (rand()  < .5)  {
...
&status("Karma string is currently \'$message2\'");
    ...
    if ($message =~ /^(8-?(breast|ball)|divine|oracle)\s+(.*)/i) {
